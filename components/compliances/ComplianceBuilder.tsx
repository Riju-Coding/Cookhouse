...
export interface ComplianceBuilderProps {
  ...
  initialCafetariaId?: string;
  onSaved?: () => void;
  onCancel?: () => void;
}

export function ComplianceBuilder({ 
  templateId, 
  initialCompanyId, 
  initialBuildingId, 
  initialCafetariaId,
  ...
  const [cafetariaId, setCafetariaId] = useState(initialCafetariaId || "")
  ...
  const fetchExistingTemplate = async () => {
    try {
      const t = await complianceTemplatesService.getById(templateId!)
      if (t) {
  ...
  const handleOpenCopyModal = async () => {
    ...
    if (copyTemplates.length === 0) {
      try {
        ...
        setCopyTemplates(templates.filter(t => t.id !== templateId))
      } catch (e) {
        console.error("Failed to load copy templates:", e)
  ...
  const handleSelectCopyTemplate = async (id: string) => {
    ...
    if (id && id !== "none") {
      try {
        ...
        console.error("Failed to load template fields for copy:", e)
  ...
  const handleBulkSubmit = () => {
    ...
    const newFields = questions.map((q, i) => ({
      question: q,
      type: 'yes_no' as TemplateFieldType,
      isRequired: true,
    ...
    setBulkText("")
    toast({ title: "Questions Added", description: `Added ${newFields.length} new questions.` })
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let saveId = templateId
      if (isNew) {
        const docRef = await complianceTemplatesService.add(payload)
        saveId = docRef.id
      } else {
        await complianceTemplatesService.update(saveId!, payload)
        // Clear existing fields
        await complianceTemplateFieldsService.deleteByTemplateId(saveId!)
      }
      ...
      if (onSaved) onSaved()
      else router.push("/admin/compliances")
    } catch (error) {
      console.error(error)
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-4">
                      ...
                      <Button variant="outline" className="flex-1 border-dashed py-6 bg-blue-50 text-blue-700 hover:bg-blue-100 hover:text-blue-800" onClick={() => setIsBulkModalOpen(true)}>
                        <List className="mr-2 h-4 w-4" /> Bulk Paste Questions
                      </Button>
      ...
      <Dialog open={isBulkModalOpen} onOpenChange={setIsBulkModalOpen}>
        <DialogContent className="max-w-xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleBulkSubmit} disabled={!bulkText.trim()}>
              Add Questions
            </Button>
          </DialogFooter>
        </DialogContent>
