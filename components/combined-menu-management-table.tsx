...
export default function CombinedMenuManagementTable() {
  ...
  const handleDelete = async (id: string) => {
    try {
      setDeleting(id)
      await deleteDoc(doc(db, "combinedMenus", id))
  ...
  const handleEdit = (id: string) => {
    router.push(`/combined-menu/${id}/edit`)
  }

  const handleCreate = () => {
    router.push("/combined-menu/new")
  }
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Combined Menus</h1>
          <p className="text-muted-foreground">Manage and edit combined menu configurations</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>All Combined Menus</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              No combined menus found. Create one to get started.
            </div>
          ) : (
                ...
                <TableBody>
                  {menus.map((menu) => (
                    <TableRow key={menu.id}>
                      ...
                      <TableCell className="text-sm">{menu.description || "-"}</TableCell>
                      <TableCell>
                        <span
